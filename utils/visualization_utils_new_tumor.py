...
BOUNDING_BOX=[]
found=False
def set_found(f_value):
    global found
    found=f_value
    BOUNDING_BOX.clear()
    Tumor_Name_List.clear()
SCORE=0
name=""
        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in category_index.keys():
              class_name ="cancer" #category_index[classes[i]]['name']
              found=True
            else:
              class_name = 'N/A'
            display_str = str(class_name)
        if not skip_scores:
          if not display_str and found==False:
            display_str = '{}%'.format(int(100*scores[i]))
          else:
            display_str = '{}: {}%'.format(display_str, int(100*scores[i]))
        Tumor_Name_List.append(display_str)
      #  display_str=''
